 ...
 * Course: CSC-7 (42833)
 * Instructor: Dr. Lehr
 * Assignment: Lab 3 Luhn Algorithm
 * This program picks a credit card number and flips 2 numbers then transposes 2 numbers from it.
 * Created on April 20, 2016, 1:44 PM
 */
...
void genCC(CrdCard select,char cc[]){
    ...
    do{
        ...
        for(z = 0; z <= 10000; z++){
            ...
            if(flipChance < 500){
            //This time Dr. Lehr wants "For each" number to be flip 2 and transpose 2 numbers
            ...
            if(z>=10000){
                cout<<endl<<endl<<"--- Validation Tracker (FLIPPED & TRANSPOSED) ---\n";
...
void transposeFlipDig(char cc[]){
    
    
    //Transposes a number a first time
    int elemTrans1st = rand()%14;
    int valTrans1st = rand()%14;
    
    //Transposes a number a second time
    int elemTrans = rand()%14; 
    int valTrans = rand()%14;
    char temp;
    
    //Flips a number a first time
    int elemFlip1st = rand()%14;
    int valFlip1st = rand()%9+48;
    
    //Flips a number a second time
    int elemFlip = rand()%14;
    int valFlip = rand()%9+48;
    
    //Flips a number a first time
    if(cc[elemFlip1st] != valFlip1st){
        cc[elemFlip1st] = valFlip1st;
    }
    ...
    if(cc[elemTrans1st] != valTrans1st){
        temp = cc[elemTrans1st];
        cc[elemTrans1st] = cc[valTrans1st];
        cc[valTrans1st] = temp;
    }

    if(cc[elemTrans] != valTrans){
        temp = cc[elemTrans];
        cc[elemTrans] = cc[valTrans];
        cc[valTrans] = temp;
                
        cout<<" --> FLIPPED 2 & TRANSPOSED 2 numbers: ";
